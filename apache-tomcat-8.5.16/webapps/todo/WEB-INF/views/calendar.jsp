	...
	$(function() {
		...
		$("#calendar-box").fullCalendar({
			...
			events: function(start, end, timezone, callback) {
				...
				var requestUrl = "my-events.do";
				
				// 우측의 사용자 목록에서 클릭된 사용자의 일정 정보를 조회하기
				if ($("#member-list li.active").length) {
					var clickedMemberId = $("#member-list li.active").attr("id").replace("member-id-", "");
					paramData['memberId'] = clickedMemberId;
					requestUrl = "other-events.do";
				} 
			...
			dayClick: function(date, jsEvent, view) {
				// 사용자 목록에 선택된 사용자가 없는 경우에만 일정등록 창을 표시한다. 사용자 목록에서 사용자가 선택되어 있는 경우는 다른 사용자의 일정을 보고 있는 중이기 때문이다.
		...
		<div class="col-xs-3">
			<div class="panel panel-default">
				...
				<div class="panel-footer text-center">
					<a href="calendar.do" class="btn btn-primary">내 일정보기</a>
				</div>
			</div>
		</div>
	</div>
